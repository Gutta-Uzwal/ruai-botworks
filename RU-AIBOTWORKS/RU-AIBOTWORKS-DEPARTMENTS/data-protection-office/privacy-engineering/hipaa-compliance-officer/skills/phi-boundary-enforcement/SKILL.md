---
name: phi-boundary-enforcement
description: "PHI never enters an agent context window — healthcare builds run on synthetic fixtures only"
...
Phi never enters an agent context window — healthcare builds run on synthetic fixtures only.

## When this applies

Invoke this when the task in front of you is phi boundary enforcement — not by default, and not
