---
name: transfer-mechanism-mapping
description: "Knowing which mechanism covers each transfer, and which sub-processor sits behind each one"
owner: data-residency-analyst
...
Knowing which mechanism covers each transfer, and which sub-processor sits behind each one.

## When this applies

Invoke this when the task in front of you is transfer mechanism mapping — not by default, and not
...
It grants no capability. If applying it requires a tool `data-residency-analyst` does not hold,
