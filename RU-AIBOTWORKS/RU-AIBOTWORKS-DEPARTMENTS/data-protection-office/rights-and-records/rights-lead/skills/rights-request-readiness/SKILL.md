---
name: rights-request-readiness
description: "Knowing where a person's data lives before someone asks, because the clock starts at the request"
owner: rights-lead
...
Knowing where a person's data lives before someone asks, because the clock starts at the request.

## When this applies

Invoke this when the task in front of you is rights request readiness — not by default, and not
...
It grants no capability. If applying it requires a tool `rights-lead` does not hold,
