---
name: mobile-screen-reader-audit
description: "Auditing with TalkBack actually running, because the accessibility tree lies on paper"
owner: mobile-accessibility-auditor
...
# mobile screen reader audit

Auditing with talkback actually running, because the accessibility tree lies on paper.

## When this applies

Invoke this when the task in front of you is mobile screen reader audit — not by default, and not
...
It grants no capability. If applying it requires a tool `mobile-accessibility-auditor` does not hold,
