---
name: on-device-secret-handling
description: "Keystore, no secrets in the bundle, and accepting that a shipped app is a decompiled app"
...
# on device secret handling

Keystore, no secrets in the bundle, and accepting that a shipped app is a decompiled app.

## When this applies

Invoke this when the task in front of you is on device secret handling — not by default, and not
...
It grants no capability. If applying it requires a tool `mobile-security-engineer` does not hold,
