---
name: device-matrix-selection
description: "Choosing the device and API-level matrix from the client's real user base, not from popularity charts"
...
Choosing the device and api-level matrix from the client's real user base, not from popularity charts.

## When this applies

Invoke this when the task in front of you is device matrix selection — not by default, and not
...
It grants no capability. If applying it requires a tool `mobile-qa-lead` does not hold,
