...
description: "Percentage rollout, halt criteria and the rollback that is really a roll-forward"
owner: play-store-release-engineer
...
Percentage rollout, halt criteria and the rollback that is really a roll-forward.

## When this applies

Invoke this when the task in front of you is staged rollout management — not by default, and not
...
It grants no capability. If applying it requires a tool `play-store-release-engineer` does not hold,
