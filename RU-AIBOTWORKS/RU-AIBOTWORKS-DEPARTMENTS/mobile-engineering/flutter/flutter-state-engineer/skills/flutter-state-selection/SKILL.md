---
name: flutter-state-selection
description: "Choosing a state solution from the app's actual shape rather than from a blog post"
owner: flutter-state-engineer
...
# flutter state selection

Choosing a state solution from the app's actual shape rather than from a blog post.

## When this applies

Invoke this when the task in front of you is flutter state selection — not by default, and not
...
It grants no capability. If applying it requires a tool `flutter-state-engineer` does not hold,
