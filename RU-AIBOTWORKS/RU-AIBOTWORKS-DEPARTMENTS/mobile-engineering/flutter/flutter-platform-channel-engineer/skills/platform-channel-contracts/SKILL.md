---
name: platform-channel-contracts
description: "Typed channel boundaries and failing gracefully when the native side is absent"
owner: flutter-platform-channel-engineer
...
Typed channel boundaries and failing gracefully when the native side is absent.

## When this applies

Invoke this when the task in front of you is platform channel contracts — not by default, and not
...
It grants no capability. If applying it requires a tool `flutter-platform-channel-engineer` does not hold,
