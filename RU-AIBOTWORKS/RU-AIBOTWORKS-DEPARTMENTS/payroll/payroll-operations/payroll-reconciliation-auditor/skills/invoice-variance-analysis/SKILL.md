---
name: invoice-variance-analysis
description: "Explaining every variance rather than absorbing it — an unexplained variance is a meter defect or a leak"
owner: payroll-reconciliation-auditor
department: payroll
...
Explaining every variance rather than absorbing it — an unexplained variance is a meter defect or a leak.

## When this applies

Invoke this when the task in front of you is invoice variance analysis — not by default, and not
...
It grants no capability. If applying it requires a tool `payroll-reconciliation-auditor` does not hold,
