---
name: period-close-discipline
description: "Closing the period only once the meter reconciles — an unreconciled meter is an estimate, not an account"
owner: payroll-ops-lead
department: payroll
...
Closing the period only once the meter reconciles — an unreconciled meter is an estimate, not an account.

## When this applies

Invoke this when the task in front of you is period close discipline — not by default, and not
...
It grants no capability. If applying it requires a tool `payroll-ops-lead` does not hold,
