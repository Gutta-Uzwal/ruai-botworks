---
name: premium-tier-flagging
description: "Flagging Fable usage explicitly at roughly 2.6x Opus, because a premium tier used quietly is the expensive kind"
owner: rate-card-administrator
department: payroll
...
Flagging fable usage explicitly at roughly 2.6x opus, because a premium tier used quietly is the expensive kind.

## When this applies

Invoke this when the task in front of you is premium tier flagging — not by default, and not
...
It grants no capability. If applying it requires a tool `rate-card-administrator` does not hold,
