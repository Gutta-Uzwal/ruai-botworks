---
name: wall-clock-ledger
description: "Elapsed time per run, separating time spent waiting on a human from time spent working"
owner: agent-hour-accountant
department: payroll
team: metering
...
Elapsed time per run, separating time spent waiting on a human from time spent working.

## When this applies

Invoke this when the task in front of you is wall clock ledger — not by default, and not
...
It grants no capability. If applying it requires a tool `agent-hour-accountant` does not hold,
