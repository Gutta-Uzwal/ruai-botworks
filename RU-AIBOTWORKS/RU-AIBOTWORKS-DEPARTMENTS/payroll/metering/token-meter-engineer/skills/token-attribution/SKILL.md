...
description: "Attributing tokens to the agent that spent them, including tokens spent by its sub-agents"
owner: token-meter-engineer
department: payroll
team: metering
...
Attributing tokens to the agent that spent them, including tokens spent by its sub-agents.

## When this applies

Invoke this when the task in front of you is token attribution — not by default, and not
...
It grants no capability. If applying it requires a tool `token-meter-engineer` does not hold,
