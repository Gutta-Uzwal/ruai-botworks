...
description: "Re-injecting and re-validating constraints at every compaction boundary, where they are otherwise silently dropped"
...
Re-injecting and re-validating constraints at every compaction boundary, where they are otherwise silently dropped.

## When this applies

Invoke this when the task in front of you is context handoff integrity — not by default, and not
...
It grants no capability. If applying it requires a tool `context-manager` does not hold,
