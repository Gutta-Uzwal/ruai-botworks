---
name: deliberate-merge-review
description: "Reading every incoming diff, because an automatic pull is an unreviewed production change"
owner: upstream-merge-reviewer
...
# deliberate merge review

Reading every incoming diff, because an automatic pull is an unreviewed production change.

## When this applies

Invoke this when the task in front of you is deliberate merge review — not by default, and not
...
It grants no capability. If applying it requires a tool `upstream-merge-reviewer` does not hold,
