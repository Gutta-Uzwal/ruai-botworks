---
name: lockfile-reconciliation
description: "Comparing installed against locked, and failing the build on any disagreement"
owner: version-drift-monitor
...
Comparing installed against locked, and failing the build on any disagreement.

## When this applies

Invoke this when the task in front of you is lockfile reconciliation — not by default, and not
...
It grants no capability. If applying it requires a tool `version-drift-monitor` does not hold,
