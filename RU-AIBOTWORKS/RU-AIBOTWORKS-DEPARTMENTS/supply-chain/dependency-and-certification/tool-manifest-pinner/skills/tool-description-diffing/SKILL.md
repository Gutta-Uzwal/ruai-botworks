---
name: tool-description-diffing
description: "Tool descriptions are untrusted text in a privileged position — an unexpected change is a supply-chain event"
owner: tool-manifest-pinner
...
# tool description diffing

Tool descriptions are untrusted text in a privileged position — an unexpected change is a supply-chain event.

## When this applies

Invoke this when the task in front of you is tool description diffing — not by default, and not
...
It grants no capability. If applying it requires a tool `tool-manifest-pinner` does not hold,
