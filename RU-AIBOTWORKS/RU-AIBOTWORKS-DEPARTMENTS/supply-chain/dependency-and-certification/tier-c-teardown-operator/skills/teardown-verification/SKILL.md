...
description: "Measuring the context baseline after teardown, because the step everyone skips is the one that rots budgets"
owner: tier-c-teardown-operator
...
Measuring the context baseline after teardown, because the step everyone skips is the one that rots budgets.

## When this applies

Invoke this when the task in front of you is teardown verification — not by default, and not
...
It grants no capability. If applying it requires a tool `tier-c-teardown-operator` does not hold,
