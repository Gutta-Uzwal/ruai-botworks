---
name: denied-attempt-logging
description: "Logging after the decision and before the effect, so denied attempts survive — the best early signal of a hijacked agent"
owner: audit-log-engineer
...
Logging after the decision and before the effect, so denied attempts survive — the best early signal of a hijacked agent.

## When this applies

Invoke this when the task in front of you is denied attempt logging — not by default, and not
...
It grants no capability. If applying it requires a tool `audit-log-engineer` does not hold,
