...
description: "Every alert names an action; one that does not is deleted rather than tuned"
...
Every alert names an action; one that does not is deleted rather than tuned.

## When this applies

Invoke this when the task in front of you is alert actionability — not by default, and not
...
It grants no capability. If applying it requires a tool `observability-lead` does not hold,
