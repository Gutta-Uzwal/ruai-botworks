---
name: subagent-and-queue-coverage
description: "Verifying the stop reaches sub-agents and queued work, not just the parent run"
...
# subagent and queue coverage

Verifying the stop reaches sub-agents and queued work, not just the parent run.

## When this applies

Invoke this when the task in front of you is subagent and queue coverage — not by default, and not
...
It grants no capability. If applying it requires a tool `kill-switch-operator` does not hold,
