---
name: error-signal-triage
description: "Separating the cause from the hundred downstream symptoms it produced"
owner: error-diagnostics-analyst
...
Separating the cause from the hundred downstream symptoms it produced.

## When this applies

Invoke this when the task in front of you is error signal triage — not by default, and not
...
It grants no capability. If applying it requires a tool `error-diagnostics-analyst` does not hold,
