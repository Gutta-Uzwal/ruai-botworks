...
description: "Answering which content steered the agent, using provenance labels rather than inference"
owner: forensics-analyst
...
Answering which content steered the agent, using provenance labels rather than inference.

## When this applies

Invoke this when the task in front of you is trajectory reconstruction — not by default, and not
...
It grants no capability. If applying it requires a tool `forensics-analyst` does not hold,
