---
name: agentic-incident-annex
description: "An IR plan that assumes a credentialed agent is the actor, and a tabletop that has been run"
...
# agentic incident annex

An ir plan that assumes a credentialed agent is the actor, and a tabletop that has been run.

## When this applies

Invoke this when the task in front of you is agentic incident annex — not by default, and not
...
It grants no capability. If applying it requires a tool `incident-response-lead` does not hold,
