---
name: notes-from-diff
description: "Notes derived from the merged diff, never from the intention recorded in the ticket"
owner: release-notes-engineer
...
Notes derived from the merged diff, never from the intention recorded in the ticket.

## When this applies

Invoke this when the task in front of you is notes from diff — not by default, and not
...
It grants no capability. If applying it requires a tool `release-notes-engineer` does not hold,
