...
description: "A freeze the runtime enforces, not one that lives in a calendar invitation"
owner: change-freeze-enforcer
...
A freeze the runtime enforces, not one that lives in a calendar invitation.

## When this applies

Invoke this when the task in front of you is freeze window enforcement — not by default, and not
...
It grants no capability. If applying it requires a tool `change-freeze-enforcer` does not hold,
