...
description: "Reading a failure well enough to route it once, correctly, without attempting the fix"
owner: release-lead
...
Reading a failure well enough to route it once, correctly, without attempting the fix.

## When this applies

Invoke this when the task in front of you is failure routing — not by default, and not
...
It grants no capability. If applying it requires a tool `release-lead` does not hold,
