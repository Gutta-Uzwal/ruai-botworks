...
description: "Running the rollback before it is needed, because an untested rollback is a hope"
...
Running the rollback before it is needed, because an untested rollback is a hope.

## When this applies

Invoke this when the task in front of you is rollback rehearsal — not by default, and not
...
It grants no capability. If applying it requires a tool `rollback-engineer` does not hold,
