...
description: "Proving a direct push to main is rejected, rather than assuming the rule is on"
owner: git-workflow-engineer
...
Proving a direct push to main is rejected, rather than assuming the rule is on.

## When this applies

Invoke this when the task in front of you is branch protection verification — not by default, and not
...
It grants no capability. If applying it requires a tool `git-workflow-engineer` does not hold,
