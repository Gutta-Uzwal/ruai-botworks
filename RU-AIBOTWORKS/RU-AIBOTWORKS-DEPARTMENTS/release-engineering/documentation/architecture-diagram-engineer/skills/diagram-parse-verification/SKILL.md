---
name: diagram-parse-verification
description: "Every Mermaid block parsed in CI, because a broken diagram fails silently in most readers"
owner: architecture-diagram-engineer
...
# diagram parse verification

Every mermaid block parsed in ci, because a broken diagram fails silently in most readers.

## When this applies

Invoke this when the task in front of you is diagram parse verification — not by default, and not
...
It grants no capability. If applying it requires a tool `architecture-diagram-engineer` does not hold,
