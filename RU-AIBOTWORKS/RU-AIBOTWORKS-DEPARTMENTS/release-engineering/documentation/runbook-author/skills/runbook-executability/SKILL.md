---
name: runbook-executability
description: "Commands that can be pasted, decision points that are binary, and no step that says investigate"
...
Commands that can be pasted, decision points that are binary, and no step that says investigate.

## When this applies

Invoke this when the task in front of you is runbook executability — not by default, and not
...
It grants no capability. If applying it requires a tool `runbook-author` does not hold,
