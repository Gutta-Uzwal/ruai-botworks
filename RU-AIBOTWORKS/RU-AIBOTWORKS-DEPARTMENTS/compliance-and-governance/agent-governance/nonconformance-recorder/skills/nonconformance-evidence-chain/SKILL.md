---
name: nonconformance-evidence-chain
description: "A finding, its evidence, its owner and its expiry — an exception with no expiry is a permanent gap"
owner: nonconformance-recorder
...
# nonconformance evidence chain

A finding, its evidence, its owner and its expiry — an exception with no expiry is a permanent gap.

## When this applies

Invoke this when the task in front of you is nonconformance evidence chain — not by default, and not
...
It grants no capability. If applying it requires a tool `nonconformance-recorder` does not hold,
