---
name: blast-radius-verification
description: "Rejecting any blast radius that is not a number and a bound"
owner: registration-record-auditor
...
Rejecting any blast radius that is not a number and a bound.

## When this applies

Invoke this when the task in front of you is blast radius verification — not by default, and not
...
It grants no capability. If applying it requires a tool `registration-record-auditor` does not hold,
