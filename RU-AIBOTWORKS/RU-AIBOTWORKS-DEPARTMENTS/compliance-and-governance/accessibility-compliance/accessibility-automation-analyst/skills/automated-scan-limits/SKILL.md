---
name: automated-scan-limits
description: "Naming the roughly two-thirds of WCAG no scanner can decide, so a green scan is not read as conformance"
owner: accessibility-automation-analyst
...
Naming the roughly two-thirds of wcag no scanner can decide, so a green scan is not read as conformance.

## When this applies

Invoke this when the task in front of you is automated scan limits — not by default, and not
...
It grants no capability. If applying it requires a tool `accessibility-automation-analyst` does not hold,
