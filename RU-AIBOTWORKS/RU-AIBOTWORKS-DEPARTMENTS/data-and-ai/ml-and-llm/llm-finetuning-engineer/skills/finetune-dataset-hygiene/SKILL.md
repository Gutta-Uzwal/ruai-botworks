---
name: finetune-dataset-hygiene
description: "Deduplication, holdout isolation and never training on the evaluation set"
owner: llm-finetuning-engineer
...
# finetune dataset hygiene

Deduplication, holdout isolation and never training on the evaluation set.

## When this applies

Invoke this when the task in front of you is finetune dataset hygiene — not by default, and not
...
It grants no capability. If applying it requires a tool `llm-finetuning-engineer` does not hold,
