...
description: "A promoted model may enter the pipeline; it has not shipped — and the difference is evaluated"
owner: ml-lead
...
A promoted model may enter the pipeline; it has not shipped — and the difference is evaluated.

## When this applies

Invoke this when the task in front of you is model promotion gate — not by default, and not
...
It grants no capability. If applying it requires a tool `ml-lead` does not hold,
