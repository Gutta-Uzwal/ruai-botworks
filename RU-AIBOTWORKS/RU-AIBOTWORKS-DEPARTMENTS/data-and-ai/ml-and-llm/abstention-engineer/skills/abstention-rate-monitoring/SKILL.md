---
name: abstention-rate-monitoring
description: "Alerting on a falling abstention rate at flat task success — the signal that confidence outran competence"
owner: abstention-engineer
...
Alerting on a falling abstention rate at flat task success — the signal that confidence outran competence.

## When this applies

Invoke this when the task in front of you is abstention rate monitoring — not by default, and not
...
It grants no capability. If applying it requires a tool `abstention-engineer` does not hold,
