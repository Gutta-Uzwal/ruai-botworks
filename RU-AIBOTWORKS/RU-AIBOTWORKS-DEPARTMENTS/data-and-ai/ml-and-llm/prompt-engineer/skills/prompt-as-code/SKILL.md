---
name: prompt-as-code
description: "Versioned prompts with an evaluation suite, because an unreviewed prompt change is an unreviewed production change"
...
Versioned prompts with an evaluation suite, because an unreviewed prompt change is an unreviewed production change.

## When this applies

Invoke this when the task in front of you is prompt as code — not by default, and not
...
It grants no capability. If applying it requires a tool `prompt-engineer` does not hold,
