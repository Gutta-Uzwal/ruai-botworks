...
description: "Reruns that produce the same result, because a pipeline will be rerun"
owner: data-engineer
...
Reruns that produce the same result, because a pipeline will be rerun.

## When this applies

Invoke this when the task in front of you is idempotent pipeline design — not by default, and not
...
It grants no capability. If applying it requires a tool `data-engineer` does not hold,
