---
name: metric-definition-governance
description: "One definition per metric, versioned, so two dashboards cannot disagree"
...
One definition per metric, versioned, so two dashboards cannot disagree.

## When this applies

Invoke this when the task in front of you is metric definition governance — not by default, and not
...
It grants no capability. If applying it requires a tool `analytics-engineer` does not hold,
