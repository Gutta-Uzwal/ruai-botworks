---
name: restore-drill-execution
description: "A backup that has never been restored is not a backup — running the drill and publishing measured RTO"
...
A backup that has never been restored is not a backup — running the drill and publishing measured rto.

## When this applies

Invoke this when the task in front of you is restore drill execution — not by default, and not
...
It grants no capability. If applying it requires a tool `database-admin` does not hold,
