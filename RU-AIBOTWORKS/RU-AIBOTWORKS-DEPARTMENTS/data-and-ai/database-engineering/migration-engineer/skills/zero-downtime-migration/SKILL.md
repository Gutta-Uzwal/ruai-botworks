...
description: "Expand-migrate-contract, and never shipping a migration whose down path was not run"
...
Expand-migrate-contract, and never shipping a migration whose down path was not run.

## When this applies

Invoke this when the task in front of you is zero downtime migration — not by default, and not
...
It grants no capability. If applying it requires a tool `migration-engineer` does not hold,
