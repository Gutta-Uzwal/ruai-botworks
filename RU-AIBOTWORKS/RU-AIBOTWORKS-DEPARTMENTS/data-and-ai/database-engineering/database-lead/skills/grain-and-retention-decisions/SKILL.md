---
name: grain-and-retention-decisions
description: "Deciding the row's meaning and its lifespan before the first INSERT, because both are migrations later"
owner: database-lead
...
# grain and retention decisions

Deciding the row's meaning and its lifespan before the first insert, because both are migrations later.

## When this applies

Invoke this when the task in front of you is grain and retention decisions — not by default, and not
...
It grants no capability. If applying it requires a tool `database-lead` does not hold,
