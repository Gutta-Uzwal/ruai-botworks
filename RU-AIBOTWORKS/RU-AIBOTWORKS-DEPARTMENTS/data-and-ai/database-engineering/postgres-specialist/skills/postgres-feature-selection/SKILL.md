---
name: postgres-feature-selection
description: "JSONB, partial indexes, CTEs and when each stops being the right tool"
owner: postgres-specialist
...
# postgres feature selection

Jsonb, partial indexes, ctes and when each stops being the right tool.

## When this applies

Invoke this when the task in front of you is postgres feature selection — not by default, and not
...
It grants no capability. If applying it requires a tool `postgres-specialist` does not hold,
