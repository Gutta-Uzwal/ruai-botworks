---
name: reproducible-build-construction
description: "Pinned toolchains and hermetic steps, so the same input yields the same artifact"
...
# reproducible build construction

Pinned toolchains and hermetic steps, so the same input yields the same artifact.

## When this applies

Invoke this when the task in front of you is reproducible build construction — not by default, and not
...
It grants no capability. If applying it requires a tool `pipeline-engineer` does not hold,
