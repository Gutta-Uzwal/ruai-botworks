---
name: no-secret-in-tool-output
description: "No tool returns a credential — not in a field, an error message or a debug payload"
owner: secrets-management-engineer
...
# no secret in tool output

No tool returns a credential — not in a field, an error message or a debug payload.

## When this applies

Invoke this when the task in front of you is no secret in tool output — not by default, and not
...
It grants no capability. If applying it requires a tool `secrets-management-engineer` does not hold,
