---
name: cache-key-correctness
description: "Keys that include everything that changes output, because a wrong hit is worse than a miss"
owner: build-cache-engineer
...
Keys that include everything that changes output, because a wrong hit is worse than a miss.

## When this applies

Invoke this when the task in front of you is cache key correctness — not by default, and not
...
It grants no capability. If applying it requires a tool `build-cache-engineer` does not hold,
