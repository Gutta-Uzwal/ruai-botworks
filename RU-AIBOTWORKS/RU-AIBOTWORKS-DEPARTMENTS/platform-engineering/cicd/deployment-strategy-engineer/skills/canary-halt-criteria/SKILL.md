---
name: canary-halt-criteria
description: "Deciding the abort signal before the rollout starts, not while watching it"
owner: deployment-strategy-engineer
...
Deciding the abort signal before the rollout starts, not while watching it.

## When this applies

Invoke this when the task in front of you is canary halt criteria — not by default, and not
...
It grants no capability. If applying it requires a tool `deployment-strategy-engineer` does not hold,
