---
name: bypass-flag-blocking
description: "Refusing --no-verify and every other gate-skipping flag, with the refusal recorded"
owner: hook-and-guardrail-engineer
...
# bypass flag blocking

Refusing --no-verify and every other gate-skipping flag, with the refusal recorded.

## When this applies

Invoke this when the task in front of you is bypass flag blocking — not by default, and not
...
It grants no capability. If applying it requires a tool `hook-and-guardrail-engineer` does not hold,
