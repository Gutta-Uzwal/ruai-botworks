---
name: policy-from-receipts
description: "Observing one full build in audit mode before writing a single rule — policy from imagination becomes an obstacle, and obstacles get loosened"
owner: policy-lead
...
# policy from receipts

Observing one full build in audit mode before writing a single rule — policy from imagination becomes an obstacle, and obstacles get loosened.

## When this applies

Invoke this when the task in front of you is policy from receipts — not by default, and not
...
It grants no capability. If applying it requires a tool `policy-lead` does not hold,
