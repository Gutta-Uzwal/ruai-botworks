---
name: irreversibility-tier-assignment
description: "The one-line test for each tier, and why downgrading one needs the same review as adding a Tier 3 tool"
owner: tool-registry-engineer
...
# irreversibility tier assignment

The one-line test for each tier, and why downgrading one needs the same review as adding a tier 3 tool.

## When this applies

Invoke this when the task in front of you is irreversibility tier assignment — not by default, and not
...
It grants no capability. If applying it requires a tool `tool-registry-engineer` does not hold,
