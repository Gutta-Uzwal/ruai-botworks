...
description: "Specific hosts not broad domains, DNS included, and an empty allowlist that fails closed"
...
Specific hosts not broad domains, dns included, and an empty allowlist that fails closed.

## When this applies

Invoke this when the task in front of you is default deny egress — not by default, and not
...
It grants no capability. If applying it requires a tool `network-engineer` does not hold,
