...
description: "Pod identity bound to a service account, so a credential never sits in a manifest"
owner: kubernetes-operator
...
Pod identity bound to a service account, so a credential never sits in a manifest.

## When this applies

Invoke this when the task in front of you is workload identity binding — not by default, and not
...
It grants no capability. If applying it requires a tool `kubernetes-operator` does not hold,
