...
description: "Scanning history as well as HEAD, because a rotated secret in history is still a leak"
owner: secrets-scanning-analyst
department: security
...
Scanning history as well as head, because a rotated secret in history is still a leak.

## When this applies

Invoke this when the task in front of you is credential leak detection — not by default, and not
...
It grants no capability. If applying it requires a tool `secrets-scanning-analyst` does not hold,
