---
name: sast-noise-reduction
description: "Tuning rules so a real finding is not buried under two hundred that are not"
owner: sast-engineer
department: security
...
Tuning rules so a real finding is not buried under two hundred that are not.

## When this applies

Invoke this when the task in front of you is sast noise reduction — not by default, and not
...
It grants no capability. If applying it requires a tool `sast-engineer` does not hold,
