---
name: stride-decomposition
description: "Walking each trust boundary rather than each feature, which is where threats actually live"
owner: threat-modeling-expert
department: security
...
Walking each trust boundary rather than each feature, which is where threats actually live.

## When this applies

Invoke this when the task in front of you is stride decomposition — not by default, and not
...
It grants no capability. If applying it requires a tool `threat-modeling-expert` does not hold,
