...
description: "Challenging every registration record's blast radius until it is a number and a bound"
owner: threat-lead
department: security
...
Challenging every registration record's blast radius until it is a number and a bound.

## When this applies

Invoke this when the task in front of you is blast radius review — not by default, and not
...
It grants no capability. If applying it requires a tool `threat-lead` does not hold,
