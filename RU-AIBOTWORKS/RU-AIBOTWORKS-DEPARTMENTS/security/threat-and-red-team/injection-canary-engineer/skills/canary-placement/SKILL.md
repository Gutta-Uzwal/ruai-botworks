...
description: "Canaries where a hijacked agent would step, and one new canary per incident as a standing output"
owner: injection-canary-engineer
department: security
...
Canaries where a hijacked agent would step, and one new canary per incident as a standing output.

## When this applies

Invoke this when the task in front of you is canary placement — not by default, and not
...
It grants no capability. If applying it requires a tool `injection-canary-engineer` does not hold,
