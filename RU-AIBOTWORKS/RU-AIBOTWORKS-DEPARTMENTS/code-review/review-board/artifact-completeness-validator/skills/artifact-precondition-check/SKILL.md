---
name: artifact-precondition-check
description: "DESIGN.md, tests, ADR and registration records present before a candidate is reviewable at all"
owner: artifact-completeness-validator
...
Design.md, tests, adr and registration records present before a candidate is reviewable at all.

## When this applies

Invoke this when the task in front of you is artifact precondition check — not by default, and not
...
It grants no capability. If applying it requires a tool `artifact-completeness-validator` does not hold,
