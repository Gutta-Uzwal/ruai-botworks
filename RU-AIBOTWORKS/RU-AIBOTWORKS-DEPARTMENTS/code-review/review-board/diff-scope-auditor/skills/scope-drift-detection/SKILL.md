...
description: "Finding the unrelated change riding along in an approved diff"
owner: diff-scope-auditor
...
Finding the unrelated change riding along in an approved diff.

## When this applies

Invoke this when the task in front of you is scope drift detection — not by default, and not
...
It grants no capability. If applying it requires a tool `diff-scope-auditor` does not hold,
