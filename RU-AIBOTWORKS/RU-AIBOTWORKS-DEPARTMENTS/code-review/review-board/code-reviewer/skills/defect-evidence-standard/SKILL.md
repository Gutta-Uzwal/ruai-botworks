---
name: defect-evidence-standard
description: "Every finding carries a concrete failure scenario — inputs, state, and the wrong outcome"
owner: code-reviewer
...
Every finding carries a concrete failure scenario — inputs, state, and the wrong outcome.

## When this applies

Invoke this when the task in front of you is defect evidence standard — not by default, and not
...
It grants no capability. If applying it requires a tool `code-reviewer` does not hold,
