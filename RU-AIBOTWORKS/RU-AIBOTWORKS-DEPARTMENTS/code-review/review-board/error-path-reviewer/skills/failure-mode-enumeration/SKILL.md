...
description: "Walking every branch that is not the happy path — the most common blocking finding here"
owner: error-path-reviewer
...
Walking every branch that is not the happy path — the most common blocking finding here.

## When this applies

Invoke this when the task in front of you is failure mode enumeration — not by default, and not
...
It grants no capability. If applying it requires a tool `error-path-reviewer` does not hold,
