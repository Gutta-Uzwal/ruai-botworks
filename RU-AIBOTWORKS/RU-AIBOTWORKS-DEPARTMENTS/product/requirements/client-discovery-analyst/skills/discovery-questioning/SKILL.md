...
description: "Questions that surface the constraint the client has not thought to mention"
owner: client-discovery-analyst
department: product
team: requirements
...
Questions that surface the constraint the client has not thought to mention.

## When this applies

Invoke this when the task in front of you is discovery questioning — not by default, and not
...
It grants no capability. If applying it requires a tool `client-discovery-analyst` does not hold,
