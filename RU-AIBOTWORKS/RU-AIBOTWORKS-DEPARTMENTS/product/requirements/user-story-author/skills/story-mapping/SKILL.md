---
name: story-mapping
description: "Mapping the user's journey before slicing it, so the first slice is usable rather than merely built"
owner: user-story-author
department: product
team: requirements
...
Mapping the user's journey before slicing it, so the first slice is usable rather than merely built.

## When this applies

Invoke this when the task in front of you is story mapping — not by default, and not
...
It grants no capability. If applying it requires a tool `user-story-author` does not hold,
