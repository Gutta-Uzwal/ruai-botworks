---
name: acceptance-evidence-collection
description: "Evidence per criterion, because an assertion that it works is not acceptance"
owner: acceptance-evidence-analyst
department: product
team: requirements
...
Evidence per criterion, because an assertion that it works is not acceptance.

## When this applies

Invoke this when the task in front of you is acceptance evidence collection — not by default, and not
...
It grants no capability. If applying it requires a tool `acceptance-evidence-analyst` does not hold,
