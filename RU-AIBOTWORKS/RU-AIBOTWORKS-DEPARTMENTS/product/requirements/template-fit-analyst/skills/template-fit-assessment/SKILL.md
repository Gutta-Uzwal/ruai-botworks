---
name: template-fit-assessment
description: "Recognising when a request is really a different template wearing this one's costume"
owner: template-fit-analyst
department: product
team: requirements
...
Recognising when a request is really a different template wearing this one's costume.

## When this applies

Invoke this when the task in front of you is template fit assessment — not by default, and not
...
It grants no capability. If applying it requires a tool `template-fit-analyst` does not hold,
