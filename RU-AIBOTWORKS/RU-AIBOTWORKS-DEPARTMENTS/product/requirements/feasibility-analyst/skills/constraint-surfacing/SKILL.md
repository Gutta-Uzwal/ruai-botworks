---
name: constraint-surfacing
description: "Saying up front that iOS cannot build on Windows, rather than letting the client find out at release"
owner: feasibility-analyst
department: product
team: requirements
...
Saying up front that ios cannot build on windows, rather than letting the client find out at release.

## When this applies

Invoke this when the task in front of you is constraint surfacing — not by default, and not
...
It grants no capability. If applying it requires a tool `feasibility-analyst` does not hold,
