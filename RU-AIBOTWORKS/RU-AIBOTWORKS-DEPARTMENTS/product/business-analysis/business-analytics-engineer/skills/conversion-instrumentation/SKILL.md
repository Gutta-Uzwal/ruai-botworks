---
name: conversion-instrumentation
description: "Measuring the event that matters commercially, with consent handled before the first beacon"
owner: business-analytics-engineer
department: product
...
Measuring the event that matters commercially, with consent handled before the first beacon.

## When this applies

Invoke this when the task in front of you is conversion instrumentation — not by default, and not
...
It grants no capability. If applying it requires a tool `business-analytics-engineer` does not hold,
