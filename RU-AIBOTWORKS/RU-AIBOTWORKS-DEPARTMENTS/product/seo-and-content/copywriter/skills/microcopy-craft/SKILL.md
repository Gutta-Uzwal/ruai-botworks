---
name: microcopy-craft
description: "Button labels, empty states and error text — the copy that decides whether a user continues"
owner: copywriter
department: product
...
Button labels, empty states and error text — the copy that decides whether a user continues.

## When this applies

Invoke this when the task in front of you is microcopy craft — not by default, and not
...
It grants no capability. If applying it requires a tool `copywriter` does not hold,
