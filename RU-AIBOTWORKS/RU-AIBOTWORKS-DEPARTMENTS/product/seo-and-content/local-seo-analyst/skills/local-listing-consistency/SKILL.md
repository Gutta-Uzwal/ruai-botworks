---
name: local-listing-consistency
description: "Name, address and phone identical everywhere, because inconsistency is what suppresses local ranking"
owner: local-seo-analyst
department: product
...
Name, address and phone identical everywhere, because inconsistency is what suppresses local ranking.

## When this applies

Invoke this when the task in front of you is local listing consistency — not by default, and not
...
It grants no capability. If applying it requires a tool `local-seo-analyst` does not hold,
