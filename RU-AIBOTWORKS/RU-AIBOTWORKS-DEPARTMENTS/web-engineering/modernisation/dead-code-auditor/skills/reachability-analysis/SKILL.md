...
description: "Proving a path is dead before deleting it, including dynamic and reflective callers"
owner: dead-code-auditor
...
Proving a path is dead before deleting it, including dynamic and reflective callers.

## When this applies

Invoke this when the task in front of you is reachability analysis — not by default, and not
...
It grants no capability. If applying it requires a tool `dead-code-auditor` does not hold,
