---
name: portable-script-authoring
description: "Scripts that fail loudly, set -euo pipefail, and behave the same on CI and a laptop"
owner: shell-tooling-engineer
...
# portable script authoring

Scripts that fail loudly, set -euo pipefail, and behave the same on ci and a laptop.

## When this applies

Invoke this when the task in front of you is portable script authoring — not by default, and not
...
It grants no capability. If applying it requires a tool `shell-tooling-engineer` does not hold,
