---
name: task-graph-design
description: "Dependency graphs and remote cache keys that make CI incremental rather than total"
owner: monorepo-architect
...
Dependency graphs and remote cache keys that make ci incremental rather than total.

## When this applies

Invoke this when the task in front of you is task graph design — not by default, and not
...
It grants no capability. If applying it requires a tool `monorepo-architect` does not hold,
