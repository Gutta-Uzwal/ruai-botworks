...
description: "Server cache versus client state, and why conflating them causes stale-data bugs"
owner: react-state-engineer
...
Server cache versus client state, and why conflating them causes stale-data bugs.

## When this applies

Invoke this when the task in front of you is client state architecture — not by default, and not
...
It grants no capability. If applying it requires a tool `react-state-engineer` does not hold,
