---
name: form-error-path-design
description: "Every failure mode surfaced to the user — the defect merge-authority blocks on most often"
owner: form-and-validation-engineer
...
# form error path design

Every failure mode surfaced to the user — the defect merge-authority blocks on most often.

## When this applies

Invoke this when the task in front of you is form error path design — not by default, and not
...
It grants no capability. If applying it requires a tool `form-and-validation-engineer` does not hold,
