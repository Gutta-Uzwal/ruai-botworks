...
description: "Static, server, streaming or client — choosing per route from measured need"
owner: nextjs-engineer
...
Static, server, streaming or client — choosing per route from measured need.

## When this applies

Invoke this when the task in front of you is rendering strategy selection — not by default, and not
...
It grants no capability. If applying it requires a tool `nextjs-engineer` does not hold,
