...
description: "Resource modelling, versioning and the errors a client can actually handle"
...
Resource modelling, versioning and the errors a client can actually handle.

## When this applies

Invoke this when the task in front of you is api contract design — not by default, and not
...
It grants no capability. If applying it requires a tool `backend-architect` does not hold,
