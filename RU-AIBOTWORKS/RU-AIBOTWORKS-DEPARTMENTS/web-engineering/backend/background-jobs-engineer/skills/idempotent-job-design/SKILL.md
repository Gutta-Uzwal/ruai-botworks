...
description: "Idempotency keys and at-least-once delivery, because retries will happen"
owner: background-jobs-engineer
...
Idempotency keys and at-least-once delivery, because retries will happen.

## When this applies

Invoke this when the task in front of you is idempotent job design — not by default, and not
...
It grants no capability. If applying it requires a tool `background-jobs-engineer` does not hold,
