---
name: backend-work-sequencing
description: "Contract first, then persistence, then handlers — so integration is not a surprise"
owner: backend-lead
...
# backend work sequencing

Contract first, then persistence, then handlers — so integration is not a surprise.

## When this applies

Invoke this when the task in front of you is backend work sequencing — not by default, and not
...
It grants no capability. If applying it requires a tool `backend-lead` does not hold,
