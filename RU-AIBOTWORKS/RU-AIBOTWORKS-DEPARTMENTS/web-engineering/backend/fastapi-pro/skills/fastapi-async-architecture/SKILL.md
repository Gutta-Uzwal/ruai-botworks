...
description: "Async boundaries, background tasks, and not blocking the loop with sync I/O"
owner: fastapi-pro
...
Async boundaries, background tasks, and not blocking the loop with sync i/o.

## When this applies

Invoke this when the task in front of you is fastapi async architecture — not by default, and not
...
It grants no capability. If applying it requires a tool `fastapi-pro` does not hold,
