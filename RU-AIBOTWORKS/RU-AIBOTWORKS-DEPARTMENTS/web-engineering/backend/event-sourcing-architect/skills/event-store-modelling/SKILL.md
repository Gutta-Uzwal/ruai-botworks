---
name: event-store-modelling
description: "Event granularity, versioning, and replays that rebuild a projection deterministically"
owner: event-sourcing-architect
...
# event store modelling

Event granularity, versioning, and replays that rebuild a projection deterministically.

## When this applies

Invoke this when the task in front of you is event store modelling — not by default, and not
...
It grants no capability. If applying it requires a tool `event-sourcing-architect` does not hold,
