...
description: "Choosing TTL, tag or write-through per access pattern, and naming the staleness you accept"
owner: caching-engineer
...
Choosing ttl, tag or write-through per access pattern, and naming the staleness you accept.

## When this applies

Invoke this when the task in front of you is cache invalidation strategy — not by default, and not
...
It grants no capability. If applying it requires a tool `caching-engineer` does not hold,
