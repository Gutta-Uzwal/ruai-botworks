---
name: jvm-concurrency-modelling
description: "Virtual threads, structured concurrency and where the old pool model still wins"
owner: java-pro
...
# jvm concurrency modelling

Virtual threads, structured concurrency and where the old pool model still wins.

## When this applies

Invoke this when the task in front of you is jvm concurrency modelling — not by default, and not
...
It grants no capability. If applying it requires a tool `java-pro` does not hold,
