---
name: event-loop-reasoning
description: "Microtasks, starvation and why an await in a loop is usually the bug"
...
Microtasks, starvation and why an await in a loop is usually the bug.

## When this applies

Invoke this when the task in front of you is event loop reasoning — not by default, and not
...
It grants no capability. If applying it requires a tool `javascript-pro` does not hold,
