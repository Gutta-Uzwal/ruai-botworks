...
description: "Extracting a voice and a visual position from a client who has only described features"
owner: brand-lead
department: design
...
Extracting a voice and a visual position from a client who has only described features.

## When this applies

Invoke this when the task in front of you is brand discovery — not by default, and not
...
It grants no capability. If applying it requires a tool `brand-lead` does not hold,
