---
name: state-coverage-design
description: "Designing empty, loading, error and partial states, which are most of what users actually see"
...
# state coverage design

Designing empty, loading, error and partial states, which are most of what users actually see.

## When this applies

Invoke this when the task in front of you is state coverage design — not by default, and not
...
It grants no capability. If applying it requires a tool `interaction-designer` does not hold,
