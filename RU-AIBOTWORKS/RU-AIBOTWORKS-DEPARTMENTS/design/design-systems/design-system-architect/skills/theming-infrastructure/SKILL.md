---
name: theming-infrastructure
description: "Light, dark and brand themes defined once, with no colour whose only definition lives in a media query"
...
Light, dark and brand themes defined once, with no colour whose only definition lives in a media query.

## When this applies

Invoke this when the task in front of you is theming infrastructure — not by default, and not
...
It grants no capability. If applying it requires a tool `design-system-architect` does not hold,
