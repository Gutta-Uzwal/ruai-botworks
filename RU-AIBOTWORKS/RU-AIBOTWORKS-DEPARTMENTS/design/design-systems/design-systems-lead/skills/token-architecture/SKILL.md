...
description: "Primitive, semantic and component tiers, so rebranding is a token change rather than a rewrite"
...
Primitive, semantic and component tiers, so rebranding is a token change rather than a rewrite.

## When this applies

Invoke this when the task in front of you is token architecture — not by default, and not
...
It grants no capability. If applying it requires a tool `design-systems-lead` does not hold,
