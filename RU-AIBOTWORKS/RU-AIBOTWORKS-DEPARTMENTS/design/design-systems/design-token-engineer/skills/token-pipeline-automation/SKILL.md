---
name: token-pipeline-automation
description: "One source generating CSS, Compose and Dart tokens, so three platforms cannot drift"
owner: design-token-engineer
department: design
team: design-systems
...
One source generating css, compose and dart tokens, so three platforms cannot drift.

## When this applies

Invoke this when the task in front of you is token pipeline automation — not by default, and not
...
It grants no capability. If applying it requires a tool `design-token-engineer` does not hold,
