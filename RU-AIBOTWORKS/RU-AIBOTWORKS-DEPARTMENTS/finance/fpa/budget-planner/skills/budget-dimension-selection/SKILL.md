---
name: budget-dimension-selection
description: "Steps, tokens, cost, wall clock, mutations and fan-out — each bounded, each failing safe"
owner: budget-planner
department: finance
team: fpa
...
# budget dimension selection

Steps, tokens, cost, wall clock, mutations and fan-out — each bounded, each failing safe.

## When this applies

Invoke this when the task in front of you is budget dimension selection — not by default, and not
...
It grants no capability. If applying it requires a tool `budget-planner` does not hold,
