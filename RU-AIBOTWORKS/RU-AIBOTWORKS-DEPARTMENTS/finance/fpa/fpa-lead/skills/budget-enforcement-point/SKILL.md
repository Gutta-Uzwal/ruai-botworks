---
name: budget-enforcement-point
description: "Every budget names where the runtime halts — a budget that warns and continues is not a budget"
owner: fpa-lead
department: finance
team: fpa
...
Every budget names where the runtime halts — a budget that warns and continues is not a budget.

## When this applies

Invoke this when the task in front of you is budget enforcement point — not by default, and not
...
It grants no capability. If applying it requires a tool `fpa-lead` does not hold,
