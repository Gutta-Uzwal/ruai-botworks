---
name: cost-per-template-modelling
description: "Full build cost including rework, because the second attempt is part of the unit cost"
owner: unit-economics-analyst
department: finance
team: fpa
...
# cost per template modelling

Full build cost including rework, because the second attempt is part of the unit cost.

## When this applies

Invoke this when the task in front of you is cost per template modelling — not by default, and not
...
It grants no capability. If applying it requires a tool `unit-economics-analyst` does not hold,
