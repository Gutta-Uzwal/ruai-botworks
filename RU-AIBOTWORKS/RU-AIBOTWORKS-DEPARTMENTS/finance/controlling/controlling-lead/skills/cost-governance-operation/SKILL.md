---
name: cost-governance-operation
description: "Daily cost ceilings per agent, and the escalation that fires before the month does"
owner: controlling-lead
department: finance
team: controlling
...
# cost governance operation

Daily cost ceilings per agent, and the escalation that fires before the month does.

## When this applies

Invoke this when the task in front of you is cost governance operation — not by default, and not
...
It grants no capability. If applying it requires a tool `controlling-lead` does not hold,
