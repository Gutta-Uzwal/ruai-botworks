---
name: spend-baseline-deviation
description: "Alerting on deviation from an agent's own baseline, because absolute thresholds miss a cheap agent going wrong"
owner: spend-anomaly-detector
department: finance
team: controlling
...
# spend baseline deviation

Alerting on deviation from an agent's own baseline, because absolute thresholds miss a cheap agent going wrong.

## When this applies

Invoke this when the task in front of you is spend baseline deviation — not by default, and not
...
It grants no capability. If applying it requires a tool `spend-anomaly-detector` does not hold,
