---
name: golden-trace-diffing
description: "Diffing agent trajectories across versions, and keeping it advisory because a flaky blocker teaches people to bypass the gate"
owner: agent-regression-engineer
...
Diffing agent trajectories across versions, and keeping it advisory because a flaky blocker teaches people to bypass the gate.

## When this applies

Invoke this when the task in front of you is golden trace diffing — not by default, and not
...
It grants no capability. If applying it requires a tool `agent-regression-engineer` does not hold,
