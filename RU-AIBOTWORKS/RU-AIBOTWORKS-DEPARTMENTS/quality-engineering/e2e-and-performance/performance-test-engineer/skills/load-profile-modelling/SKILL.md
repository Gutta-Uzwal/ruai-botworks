---
name: load-profile-modelling
description: "Modelling the client's real traffic shape instead of a flat ramp that tests nothing"
...
Modelling the client's real traffic shape instead of a flat ramp that tests nothing.

## When this applies

Invoke this when the task in front of you is load profile modelling — not by default, and not
...
It grants no capability. If applying it requires a tool `performance-test-engineer` does not hold,
