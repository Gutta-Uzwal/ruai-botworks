---
name: test-suite-scoping
description: "Choosing the layer each behaviour is proved at, so one defect fails one test rather than forty"
...
Choosing the layer each behaviour is proved at, so one defect fails one test rather than forty.

## When this applies

Invoke this when the task in front of you is test suite scoping — not by default, and not
...
It grants no capability. If applying it requires a tool `test-engineering-lead` does not hold,
