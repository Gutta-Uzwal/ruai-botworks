...
description: "Catching a breaking API change at the provider's build, not at the consumer's incident"
owner: contract-test-engineer
...
Catching a breaking api change at the provider's build, not at the consumer's incident.

## When this applies

Invoke this when the task in front of you is consumer contract verification — not by default, and not
...
It grants no capability. If applying it requires a tool `contract-test-engineer` does not hold,
