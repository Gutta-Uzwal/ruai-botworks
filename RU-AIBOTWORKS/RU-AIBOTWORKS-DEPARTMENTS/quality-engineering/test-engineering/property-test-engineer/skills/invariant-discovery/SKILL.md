...
description: "Finding the property that must always hold, then letting the generator attack it"
owner: property-test-engineer
...
Finding the property that must always hold, then letting the generator attack it.

## When this applies

Invoke this when the task in front of you is invariant discovery — not by default, and not
...
It grants no capability. If applying it requires a tool `property-test-engineer` does not hold,
