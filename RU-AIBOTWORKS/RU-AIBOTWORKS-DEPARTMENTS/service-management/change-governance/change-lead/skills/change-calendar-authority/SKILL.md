---
name: change-calendar-authority
description: "A freeze the policy plane enforces, because the most-cited incident happened during one that was only announced"
owner: change-lead
...
A freeze the policy plane enforces, because the most-cited incident happened during one that was only announced.

## When this applies

Invoke this when the task in front of you is change calendar authority — not by default, and not
...
It grants no capability. If applying it requires a tool `change-lead` does not hold,
