---
name: catalogue-boundary-keeping
description: "Recording declined domains explicitly, so a salesperson cannot promise one by accident"
owner: service-catalogue-curator
...
# catalogue boundary keeping

Recording declined domains explicitly, so a salesperson cannot promise one by accident.

## When this applies

Invoke this when the task in front of you is catalogue boundary keeping — not by default, and not
...
It grants no capability. If applying it requires a tool `service-catalogue-curator` does not hold,
