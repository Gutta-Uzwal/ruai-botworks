...
description: "Every request owned by a name and closed against evidence, never closed for age"
owner: request-fulfilment-coordinator
...
Every request owned by a name and closed against evidence, never closed for age.

## When this applies

Invoke this when the task in front of you is fulfilment tracking — not by default, and not
...
It grants no capability. If applying it requires a tool `request-fulfilment-coordinator` does not hold,
