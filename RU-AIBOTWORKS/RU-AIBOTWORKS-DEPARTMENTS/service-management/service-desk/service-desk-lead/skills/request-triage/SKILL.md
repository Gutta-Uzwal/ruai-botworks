---
name: request-triage
description: "Separating a request, an incident and a change at intake, because each has a different clock"
...
Separating a request, an incident and a change at intake, because each has a different clock.

## When this applies

Invoke this when the task in front of you is request triage — not by default, and not
...
It grants no capability. If applying it requires a tool `service-desk-lead` does not hold,
