---
name: registration-record-authoring
description: "Purpose, classification, tools, max tier, budgets, blast radius and incident clocks — complete or the agent does not run"
owner: agent-registrar
...
# registration record authoring

Purpose, classification, tools, max tier, budgets, blast radius and incident clocks — complete or the agent does not run.

## When this applies

Invoke this when the task in front of you is registration record authoring — not by default, and not
...
It grants no capability. If applying it requires a tool `agent-registrar` does not hold,
