---
name: lifecycle-stage-gating
description: "No agent runs without a registration record; none is retired without its credentials revoked and its record closed"
owner: agent-lifecycle-lead
...
No agent runs without a registration record; none is retired without its credentials revoked and its record closed.

## When this applies

Invoke this when the task in front of you is lifecycle stage gating — not by default, and not
...
It grants no capability. If applying it requires a tool `agent-lifecycle-lead` does not hold,
