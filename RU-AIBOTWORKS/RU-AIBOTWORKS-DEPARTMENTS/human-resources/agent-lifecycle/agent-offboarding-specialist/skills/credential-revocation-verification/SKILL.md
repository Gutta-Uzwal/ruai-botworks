---
name: credential-revocation-verification
description: "Proving the revoked credential no longer works, rather than trusting the revoke call returned success"
owner: agent-offboarding-specialist
...
Proving the revoked credential no longer works, rather than trusting the revoke call returned success.

## When this applies

Invoke this when the task in front of you is credential revocation verification — not by default, and not
...
It grants no capability. If applying it requires a tool `agent-offboarding-specialist` does not hold,
