...
1. **One source of truth.** All company data — officers, departments, teams, agents,
   tools, promotions — lives in `RU-AIBOTWORKS-REGISTRY/` as reviewable YAML. Nothing
...
2. **Artifacts are generated, never hand-edited.** Agent charters, registration records,
   ...
   `RU-AIBOTWORKS-PLATFORM/` from the registry. Editing a generated file is a defect.
